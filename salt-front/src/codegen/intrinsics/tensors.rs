...
pub fn emit_tensor_intrinsic(
    ...
    match name {
        ...
        "add_bias" => {
             ...
             out.push_str(&format!("    scf.for {} = arith.constant 0 : index to {} step arith.constant 1 : index {{\n", loop_idx, size_idx));
             let d_ptr = format!("%d_ptr_{}", id);
             let b_ptr = format!("%b_ptr_{}", id);
             out.push_str(&format!("      {} = llvm.getelementptr {}[{}] : (!llvm.ptr, index) -> !llvm.ptr, f32\n", d_ptr, dst_ptr, loop_idx));
             out.push_str(&format!("      {} = llvm.getelementptr {}[{}] : (!llvm.ptr, index) -> !llvm.ptr, f32\n", b_ptr, bias_ptr, loop_idx));
